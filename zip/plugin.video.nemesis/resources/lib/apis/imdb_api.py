...
def clear_imdb_cache(silent=False):
	...
	try:
		IMDB_DATABASE = translate_path('special://profile/addon_data/plugin.video.nemesis/databases/maincache.db')
		if not path_exists(IMDB_DATABASE): return True
		dbcon = database.connect(IMDB_DATABASE, timeout=40.0, isolation_level=None)
		dbcur = dbcon.cursor()
