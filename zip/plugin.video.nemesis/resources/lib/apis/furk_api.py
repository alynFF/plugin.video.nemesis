...
class FurkAPI:
	...
	def search(self, query, expiration=48):
		try:
			...
			string = 'nemesis_FURK_SEARCH_%s' % url
	...
	def direct_search(self, query):
		try:
			...
			string = 'nemesis_FURK_SEARCH_DIRECT_%s' % url
	...
	def t_files(self, file_id):
		try:
			...
			string = 'nemesis_%s_%s' % ('FURK_T_FILE', file_id)
...
def clear_media_results_database():
	...
	dbcon = database.connect(translate_path('special://profile/addon_data/plugin.video.nemesis/databases/maincache.db'), timeout=40.0, isolation_level=None)
	dbcur = dbcon.cursor()
	dbcur.execute('''PRAGMA synchronous = OFF''')
	dbcur.execute('''PRAGMA journal_mode = OFF''')
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'nemesis_FURK_SEARCH_%'")
	try:
		furk_results = [str(i[0]) for i in dbcur.fetchall()]
		if not furk_results: return 'success'
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'nemesis_FURK_SEARCH_%'")
