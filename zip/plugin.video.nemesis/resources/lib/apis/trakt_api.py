...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page=1, suppress_error_notification=False):
	...
	def send_query():
		...
		try:
			if method:
				if method == 'post': resp = requests.post(API_ENDPOINT % path, headers=headers, timeout=timeout)
				elif method == 'delete': resp = requests.delete(API_ENDPOINT % path, headers=headers, timeout=timeout)
				elif method == 'sort_by_headers': resp = requests.get(API_ENDPOINT % path, params=params, headers=headers, timeout=timeout)
...
def trakt_collection_lists(media_type, param1, param2):
	...
	window_property_name = 'nemesis_trakt_collection_%s' % string_insert
...
def make_new_trakt_list(params):
	...
	mode = params['mode']
	list_title = kodi_utils.dialog.input('Nemesis')
