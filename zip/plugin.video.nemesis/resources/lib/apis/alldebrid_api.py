...
class AllDebridAPI:
	...
	def clear_cache(self):
		try:
			AD_DATABASE = kodi_utils.translate_path('special://profile/addon_data/plugin.video.nemesis/databases/maincache.db')
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('nemesis_ad_user_cloud',))
				kodi_utils.clear_property('nemesis_ad_user_cloud')
				dbcon.commit()
				user_cloud_success = True
			except: user_cloud_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('nemesis_ad_valid_hosts',))
				kodi_utils.clear_property('nemesis_ad_valid_hosts')
				dbcon.commit()
				dbcon.close()
