...
class Discover:
	def __init__(self, params):
		...
		if self.media_type: self.window_id = 'Nemesis_%s_discover_params' % self.media_type.upper()
		else: self.window_id = ''
	...
	def movie(self):
		...
		if not any(i in names for i in ('similar', 'recommended')):
			...
			self._add_dir({'mode': 'discover.include_genres', 'media_type': 'movie', 'list_name': self.base_str % (self.include_base_str % ls(32470), names.get('include_genres', ''))})
			self._add_dir({'mode': 'discover.exclude_genres', 'media_type': 'movie', 'list_name': self.base_str % (self.exclude_base_str % ls(32470), names.get('exclude_genres', ''))})
	...
	def tvshow(self):
		...
		if not any(i in names for i in ['similar', 'recommended']):
			...
			self._add_dir({'mode': 'discover.include_genres', 'media_type': 'tvshow', 'list_name': self.base_str % (self.include_base_str % ls(32470), names.get('include_genres', ''))})
			self._add_dir({'mode': 'discover.exclude_genres', 'media_type': 'tvshow', 'list_name': self.base_str % (self.exclude_base_str % ls(32470), names.get('exclude_genres', ''))})
	...
	def history(self, media_type=None, display=True):
		...
		cache_file = kodi_utils.translate_path('special://profile/addon_data/plugin.video.nemesis/databases/maincache.db')
		media_type = media_type if media_type else self.media_type
		string = 'nemesis_discover_%s_%%' % media_type
		dbcon = database.connect(cache_file, timeout=40.0, isolation_level=None)
		dbcur = dbcon.cursor()
	...
	def help(self):
		text_file = kodi_utils.translate_path('special://home/addons/plugin.video.nemesis/resources/text/tips/135. Nemesis Discover.txt')
	...
	def _build_url(self, query):
		return ''.join(['plugin://plugin.video.nemesis/?', urlencode(query)])
...
def set_history(media_type, name, query):
	...
	string = 'nemesis_discover_%s_%s' % (media_type, query)
