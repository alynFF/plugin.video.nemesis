...
icon = kodi_utils.translate_path('special://home/addons/plugin.video.nemesis/icon.png')
fanart = kodi_utils.translate_path('special://home/addons/plugin.video.nemesis/fanart.png')
...
def trailer_choice(media_type, poster, tmdb_id, trailer_url, all_trailers=[]):
	...
	if all_trailers:
		...
		if len(all_trailers) == 1:
			...
			video_id = kodi_utils.select_dialog([i['key'] for i in all_trailers], **kwargs)
...
def genres_choice(media_type, genres, poster, return_genres=False):
	...
	return kodi_utils.select_dialog([{'mode': mode, 'action': action, 'genre_id': i['value'][0]} for i in genre_list], **kwargs)
...
def imdb_keywords_choice(media_type, imdb_id, poster):
	...
	kodi_utils.hide_busy_dialog()
	keyword_choice = kodi_utils.select_dialog([{'mode': mode, 'action': 'imdb_keywords_list_contents', 'list_id': i, 'media_type': media_type} for i in keywords_info], **kwargs)
...
def imdb_videos_choice(videos, poster):
	...
	return kodi_utils.select_dialog([i['url'] for i in videos], **kwargs)
...
def trakt_manager_choice(params):
	...
	icon = params.get('icon', None) or kodi_utils.translate_path('special://home/addons/script.tikiart/resources/media/trakt.png')
...
def set_quality_choice(quality_setting):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Nemesis', 'enumerate': 'false', 'multi_choice': 'true', 'multi_line': 'false', 'preselect': preselect}
...
def folder_scraper_manager_choice(folder_info=None):
	def _get_property(setting_id):
		return kodi_utils.get_property('nemesis_%s' % setting_id) or get_setting(setting_id)
	def _set_property(setting_id, setting_value):
		kodi_utils.set_property('nemesis_%s' % setting_id, setting_value)
	...
	def _exit_save_settings():
		for folder_no in range(1,6):
			...
			_clear_property('nemesis_%s' % name_setting % folder_no)
			_clear_property('nemesis_%s' % movie_dir_setting % folder_no)
			_clear_property('nemesis_%s' % tvshow_dir_setting % folder_no)
	...
	def _set_folder_path():
		if _get_property(setting) not in ('', 'None'):
			...
			kwargs = {'items': json.dumps(list_items), 'heading': 'Nemesis', 'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'false'}
			...
			if action == 1:
				...
				folder = kodi_utils.dialog.browse(0, 'Nemesis', '')
...
def color_choice(msg_dialog='Nemesis', no_color=False):
...
def favorites_choice(params):
	...
	if any(i['tmdb_id'] == tmdb_id for i in current_favourites): action, text = favourites.remove_from_favourites, '%s Nemesis %s?' % (ls(32603), ls(32453))
	else: action, text = favourites.add_to_favourites, '%s Nemesis %s?' % (ls(32602), ls(32453))
	if not kodi_utils.confirm_dialog(heading='Nemesis - %s' % title, text=text, top_space=True): return
	action()
...
def external_scrapers_choice():
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'Nemesis', 'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'true'}
...
def options_menu(params, meta=None):
	...
	results_sorting_status = get_setting('results.sort_order_display').replace('$ADDON[plugin.video.nemesis 32582]', ls(32582))
	...
	listing += [('%s %s %s' % (open_str, ls(32036), settings_str), '', 'open_nemesis_settings')]
	...
	elif choice == 'open_nemesis_settings': return open_settings('0.0')
