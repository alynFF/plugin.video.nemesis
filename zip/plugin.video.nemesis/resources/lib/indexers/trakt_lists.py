...
def search_trakt_lists(params):
	...
	search_title = params.get('search_title', None) or kodi_utils.dialog.input('Nemesis')
...
def build_trakt_list(params):
	...
	movie_list = [i[i['type']]['ids'] for i in result if i['type'] == 'movie']
	tvshow_list = [i[i['type']]['ids'] for i in result if i['type'] == 'show']
	...
	item_list = function({'list': process_list, 'id_type': 'trakt_dict'}).worker()
	item_list.sort(key=lambda k: int(k[1].getProperty('nemesis_sort_order')))
