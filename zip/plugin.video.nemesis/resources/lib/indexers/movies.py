...
class Movies:
	...
	def fetch_list(self):
		try:
			...
			if self.action in tmdb_main:
				...
				genre_id = params_get('genre_id') if 'genre_id' in self.params else self.multiselect_genres(params_get('genre_list'))
	...
	def build_movie_content(self, item_position, _id):
		try:
			...
			set_property('nemesis_sort_order', string(item_position))
			if self.is_widget:
				...
				set_property('nemesis_trakt_manager_params', trakt_manager_params)
				set_property('nemesis_fav_manager_params', fav_manager_params)
	...
	def worker(self):
		...
		self.watched_title = 'Trakt' if self.watched_indicators == 1 else 'Nemesis'
		...
		self.items.sort(key=lambda k: int(k[1].getProperty('nemesis_sort_order')))
		return self.items
