...
NAVIGATOR_DB = tp('special://profile/addon_data/plugin.video.nemesis/databases/navigator.db')
icon_directory, fanart = 'special://home/addons/script.tikiart/resources/media/%s', tp('special://home/addons/plugin.video.nemesis/fanart.png')
...
class Navigator:
	...
	def settings(self):
		...
		nemesis_str, fenom_scr_str, myaccounts_str = ls(32036), ls(32522), ls(33025)
		n_ins, lst_ins = _in_str % (settings_str.upper(), '%s'), (_in_str % (settings_str, '%s')).replace('[B]', '').replace(': [/B]', ' ')
		self.AD({'mode': 'open_settings', 'query': '0.0', 'list_name': lst_ins % nemesis_str}, n_ins % nemesis_str, 'settings.png', False)
	...
	def log_utils(self):
		...
		nemesis_str, cl_str, fs_str, ma_str, lut_str, k_str = ls(32036), ls(32508), ls(32522), ls(33025), ls(32777), ls(32538)
		lv_str, lu_str = ls(32509), ls(32853)
		nemesis_vstr = addon().getAddonInfo('version')
		sc_v, ma_v = addon('script.module.fenomscrapers').getAddonInfo('version'), addon('script.module.myaccounts').getAddonInfo('version')
		mt_str, mh_str = tp(log_path % 'plugin.video.nemesis/resources/text'), '[B]%s[/B]: %s  [I](v.%s)[/I]' % (cl_str.upper(), nemesis_str, nemesis_vstr)
	...
	def tips(self):
		tips_location = 'special://home/addons/plugin.video.nemesis/resources/text/tips'
	...
	def because_you_watched(self):
		...
		def _convert_nemesis_watched_episodes_info(watched_indicators):
			final_list, used_names = [], []
			final_append, used_append = final_list.append, used_names.append
		...
		function = get_watched_info_movie if media_type == 'movie' else _convert_nemesis_watched_episodes_info
		(key_index, name_index, tmdb_index) = (2, 1, 0) if media_type == 'movie' else (4, 3, 0)
		...
		for item in recently_watched:
			...
			if media_type == 'movie': name = because_ins % item[name_index]
			else:
