...
dbfile = translate_path('special://profile/addon_data/plugin.video.nemesis/databases/metacache.db')
...
class MetaCache:
	...
	def delete_all(self):
		try:
			...
			for i in all_entries:
				try:
					self.delete_memory_cache(str(i[0]), 'tmdb_id', string(i[1]))
					self.delete_all_seasons_memory_cache(string(i[1]))
				except: pass
		except: return
	...
	def _connect_database(self):
		return database.connect(dbfile, timeout=240, isolation_level=None)
