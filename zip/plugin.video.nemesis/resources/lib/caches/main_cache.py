...
dbfile = translate_path('special://profile/addon_data/plugin.video.nemesis/databases/maincache.db')
...
class MainCache(BaseCache):
	...
	def delete_all_folderscrapers(self):
		dbcon = self.connect_database()
		dbcur = self.set_PRAGMAS(dbcon)
		dbcur.execute(like_select % "'nemesis_FOLDERSCRAPER_%'")
		remove_list = [str(i[0]) for i in dbcur.fetchall()]
		if not remove_list: return 'success'
		try:
			dbcur.execute(like_delete % "'nemesis_FOLDERSCRAPER_%'")
			dbcon.execute('VACUUM')
