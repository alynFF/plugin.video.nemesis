...
def open_settings(query, addon='plugin.video.nemesis'):
...
def clean_settings():
	...
	addon_ids = ('plugin.video.nemesis', 'script.module.fenomscrapers', 'script.module.myaccounts')
...
def toggle_language_invoker():
	...
	kodi_utils.close_all_dialog()
	kodi_utils.sleep(100)
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.nemesis/addon.xml')
...
def clear_all_cache():
	...
	line = '[CR]%s....[CR]%s'
	kodi_utils.progressDialog.create('Nemesis', '')
...
def refresh_artwork():
	...
	for item in ('icon.png', 'fanart.png'):
		try:
			icon_path = kodi_utils.translate_path('special://home/addons/plugin.video.nemesis/%s' % item)
...
def upload_logfile():
	...
	try:
		...
		UserAgent = 'Nemesis %s' % kodi_utils.addon().getAddonInfo('version')
