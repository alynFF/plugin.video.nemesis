...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self._update_meta()
		self._search_info()
		set_property('nemesis_playback_meta', json.dumps(self.meta))
		self.get_sources()
	...
	def play_execute_background(self, results):
		...
		set_property('nemesis_background_url', background_url)
	...
	def furkPacks(self, name, file_id, highlight=None, download=False):
		...
		return NemesisPlayer().run(link, 'video')
	...
	def play_file(self, results, source={}, autoplay=False, background=False):
		...
		try:
			...
			return NemesisPlayer().run(self.url)
		except: pass
