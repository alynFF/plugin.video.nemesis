...
fanart_empty = kodi_utils.translate_path('special://home/addons/plugin.video.nemesis/fanart.png')

class NemesisPlayer(kodi_utils.xbmc_player):
	def __init__ (self):
	...
	def run(self, url=None, media_type=None):
		...
		try:
			...
			self.meta = json.loads(kodi_utils.get_property('nemesis_playback_meta'))
			...
			else: bookmark = self.bookmarkNemesis()
	...
	def bookmarkNemesis(self):
		bookmark = 0
		watched_indicators = settings.watched_indicators()
		try: resume_point, curr_time = indicators.detect_bookmark(indicators.get_bookmarks(self.media_type, watched_indicators), self.tmdb_id, self.season, self.episode)
	...
	def media_watched_marker(self):
		...
		try:
			if self.current_point >= self.set_watched:
				...
				kodi_utils.clear_property('nemesis_nextep_autoplays')
				kodi_utils.clear_property('nemesis_random_episode_history')
				if self.current_point >= self.set_resume: indicators.set_bookmark(self.media_type, self.tmdb_id, self.curr_time, self.total_time, self.season, self.episode)
		except: pass
...
class Subtitles(kodi_utils.xbmc_player):
	...
	def get(self, query, imdb_id, season, episode):
		...
		sub_filename = 'NemesisSubs_%s_%s_%s' % (imdb_id, season, episode) if season else 'NemesisSubs_%s' % imdb_id
