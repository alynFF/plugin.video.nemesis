...
__addon__ = Addon(id='plugin.video.nemesis')
...
player, xbmc_player, numeric_input = xbmc.Player(), xbmc.Player, xbmcgui.INPUT_NUMERIC
...
def addon(addon_id='plugin.video.nemesis'):
...
def end_directory(handle, cache=False):
...
def external_browse():
	return 'nemesis' not in get_infolabel('Container.PluginName')
...
def ok_dialog(heading='Nemesis', text='', highlight='royalblue', ok_label=local_string(32839), top_space=False):
...
def confirm_dialog(heading='Nemesis', text='', highlight='royalblue', ok_label=local_string(32839), cancel_label=local_string(32840), top_space=False, default_control=11):
	from windows import open_window
	if isinstance(heading, int): heading = local_string(heading)
	if isinstance(text, int): text = local_string(text)
...
def show_text(heading, text=None, file=None, font_size='small', kodi_log=False):
	...
	if kodi_log and confirm_dialog(heading='Nemesis', text=local_string(32855), ok_label=local_string(32824), cancel_label=local_string(32828), top_space=True):
...
def notification(line1, time=5000, icon=None, sound=False):
	...
	icon = icon or translate_path('special://home/addons/plugin.video.nemesis/icon.png')
	xbmcgui.Dialog().notification('Nemesis', line1, icon, time, sound)
...
def choose_view(view_type, content):
	...
	fanart = translate_path('special://home/addons/plugin.video.nemesis/fanart.png')
...
def set_view(view_type):
	...
	dbcon = database.connect(translate_path('special://profile/addon_data/plugin.video.nemesis/databases/views.db'), timeout=40.0, isolation_level=None)
	dbcur = dbcon.cursor()
...
def set_view_property(view_type, view_id):
	set_property('nemesis_%s' % view_type, view_id)
...
def set_view_properties():
	...
	for item in view_ids: set_property('nemesis_%s' % item[0], item[1])

def set_view_mode(view_type, content='files'):
	if external_browse(): return
	view_id = get_property('nemesis_%s' % view_type)
	hold = 0
	if not view_id:
		try:
			dbcon = database.connect(translate_path('special://profile/addon_data/plugin.video.nemesis/databases/views.db'), timeout=40.0, isolation_level=None)
...
def build_url(url_params):
	return ''.join(['plugin://plugin.video.nemesis/?', urlencode(url_params)])

def add_dir(url_params, list_name, __handle__, iconImage='DefaultFolder.png', fanartImage=None, isFolder=True):
	fanart = fanartImage or translate_path('special://home/addons/plugin.video.nemesis/fanart.png')
