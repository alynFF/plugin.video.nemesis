...
class Extras(BaseDialog):
		...
		if action in self.context_actions:
			...
			if focus_id == playbrowse_id and self.media_type == 'movie':
				...
				params = {'action': 'image', 'name': '%s %s' % (self.rootname, chosen_listitem.getProperty('tikiskins.extras.name')),
						...
						'media_type': 'image', 'image': translate_path('special://home/addons/plugin.video.nemesis/icon.png')}
	...
	def get_progress(self):
		try: resume_point, curr_time = watched_status.detect_bookmark(watched_status.get_bookmarks('movie', self.watched_indicators), self.tmdb_id)
